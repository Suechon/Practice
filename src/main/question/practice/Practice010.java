...
/*
* 問1.以下の内容で3種n類のRetryableLogicの実装クラスを作成する。
...
*   モッククラス-2.String型を引数にとり、インスタンス生成以降2回目まではRetryableExceptionをスローし、3回目以降は(RetryableExceptionでない)eExceptionをスローする。
* 問5.以下のリファクタリングを行う。
*    5-1.問1で作ったクラスから共通部分を抜き出した抽象クラスを作る。
*    5-2.問1で作ったクラスを4-1で作った抽象クラスを継承する形に修正する。
*/
